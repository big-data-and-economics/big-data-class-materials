...
```{r gsheet2tbl, cache=F, message=F}
gsheet2tbl
```
...
```{r construct_download_url, cache=F, message=F}
...
You might be tempted to just immediately use the `html_table` function again. But if we try that, what happens? We don't get a table. We get 116 empty tibbles because we have not yet added structure.
...
## Extracting the URL
Finally, to get the URL, notice that this is an attribute within the `<a>` tag. We simply select this node with the `html_element()` function, and then select the href attribute with the `html_attr()` function.
