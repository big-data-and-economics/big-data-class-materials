...
- Navigate to [https://tinyurl.com/islrecon368](https://emilhvitfeldt.github.io/ISLR-tidymodels-labs/05-resampling-methods.html), a website with interactive demos of machine learning resampling from the textbook Introduction to Statistical Learning
...
**Demo**: [https://tinyurl.com/islrecon368](https://emilhvitfeldt.github.io/ISLR-tidymodels-labs/05-resampling-methods.html)

1. Plot the relationship between mpg and horsepower as shown below. Would a straight-line or curved-line fit better? Why?
```{r plot-mpg-horsepower, eval=FALSE}
ggplot(Auto,aes(x=mpg,y=horsepower)) + geom_point()
```

#### 5.1 (skip 5.2)

2. What is `set.seed()`? Why is it important to use it when splitting the sample?
3. Why can't you test your model on the training data?

#### 5.3

4. Why did you replace degree with the function `tune()`?
5. How does k-fold cross-validation allow you to tune your model? 
6. Why does RSME (R<sup>2</sup>) start to increase (decrease) after two polynomials?

#### 5.4

6. Use `View(broom::tidy(Portfolio_boots$splits[[1]]))` to see the first bootstrap sample. What do you think "Analysis" and "Assessment" can be used for?  
7. Try using bootstrapping to return standard deviation for the polynomial model of auto. _Hint: Use the `poly_wf`_. 
