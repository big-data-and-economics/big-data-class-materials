...
- Navigate to the folder `big-data-class-materials/lectures/03-data-tips/`

- Double-click the file `data-tips.Rproj` to open the project in RStudio at the correct working directory
...
bp <- read_csv('data/messier_bp.csv',skip=4,
    col_names=c('pat_id','birth_month','birth_day','birth_year','race','sex','hispanic',
    'bp_visit1','hr_visit1','bp_visit2','hr_visit2','bp_visit3','hr_visit3'),show_col_types = FALSE)
...
# Get the first three rows of the data frame (or as many rows as needed)
#Make a density of the heart rate on visit 1:
bp %>% # pipes are a greate tool, we'll cover them in a bit
  mutate(race=ifelse(race=='WHITE' | race=='Caucasian','White',race)) %>% # change the data up
  ggplot(aes(x=hr_visit1,fill=race))+geom_density()
```
