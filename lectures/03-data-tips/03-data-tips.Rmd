...
**Note**: I put data files on Github. This is bad practice for large files, but fine for small ones. I recommend you add a data file extensions (e.g. *.csv, *.dta) for data files to your `.gitignore` file.
...
```{r skip-rows}
resumes <- read_csv('data/lakisha_aer.csv',skip=4,
...
- This disproportionately occurs for fake black name applicant profiles
...
See slides from [Lecture 1](https://raw.githack.com/big-data-and-economics/big-data-class-materials/main/lectures/01-intro/01-Intro.html#58) for a full explanations
...
read_csv(unz('data/lakisha_aer.zip','lakisha_aer.csv'),show_col_types = FALSE)
```
