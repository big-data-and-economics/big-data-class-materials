...
.hi[GUI:] A Graphical User Interface -- what you're used to pointing and clicking to navigate a computer and execute programs
...
.footnote[<sup>1</sup> Handle in Git lecture notes.]
...
- When you are running a script, you are working from a particular directory
  ...
  - In **R**, you can see what directory you are in using the `getwd()` function
    ...
    - You can change your working directory using the `setwd()` function (avoid this within scripts)
    - Alternatively, you can double click the `.Rproj` file to set the working directory to the root of the project

```{r directories,collapse=TRUE}
getwd()
...
.hi[Relative paths] solve a lot of this:
  ...
  - **This is portable** -- if I send this script to you and you have a copy of the `big-data-class-materials` repository, this script will work

.footnote[<sup>1</sup> This is a Windows path, Mac and Linux paths use `/` instead of `\`.]
...
# Instructions:
# Run this code from the root directory of your project where the housekeeping file, build, and analysis files are located
...
- It improves chances your code runs from start to finish instead of only when you are working interactively
  - Means I can run your code more easily! 
...
- What is abstraction? "reducing the complexity of something by hiding unnecessary details from the user"
...
- Code can be "self-documenting" by leveraging abstraction: name your functions and variables thoughtfully
...
- For my personal task management, I use GitHub Projects
...
- If your code throws an error message, there's likely a ton of superfluous stuff in your code that is not causing the error
  - But the superfluous stuff makes it harder to read!
...
- [Minimal reproducible examples](https://stackoverflow.com/help/minimal-reproducible-example) (reprex) are a great way to isolate the error
    - .hi[Minimal]:: Use as little code as possible that still produces the same problem
    - .hi[Complete]:: Provide all parts needed to reproduce your problem in the question itself
    - .hi[Reproducible]:: Test the code you'll provide to make sure it reproduces the problem
...
- Sync your fork of the exercise repository and open the folder for [mre-exercise](https://github.com/big-data-and-economics/exercises/blob/main/mre-exercise)
...
- Try to write an MRE
- I have already raised this as a poorly-written issue on GitHub. You can see the issue [here](https://github.com/big-data-and-economics/exercises/issues/6)
- Tips: https://aosmith16.github.io/spring-r-topics/slides/week09_reprex.html#1

```{r mre, error=TRUE}
# install.packages('ggplot2')
library(ggplot2)
opp_atlas <- read.csv('https://raw.githubusercontent.com/big-data-and-economics/exercises/main/mre-exercise/data/county_opatlas.csv?token=GHSAT0AAAAAACVGS6MILY55LOABVIMH4OX6ZXDI4DQ')
opp_atlas_me = opp_atlas[opp_atlas$state_name=='Maine'] 

opp_atlas_andro_mean = mean(opp_atlas_me[opp_atlas_me$county_name=='Androscoggin','kfr_p25'],na.rm=TRUE)

ggplot(data=opp_atlas_me, aes(x=cohort, y=kfr_p25)) +
    geom_point(aes(col=county_name)) +
    geom_smooth(method='lm',se=FALSE) +
    geom_hline(yintercept=opp_atlas_andro_mean, color='red') +
    annotate('text', x=1985,y=opp_atlas_andro_mean-.005, label='Androscoggin', color='red') +
    labs(title='Average income percentile of children born in 25th income percentile by cohort', x='Cohort', y='Income percentile',col='County')
