...
2. [AI and the Empirical Workflow](#ai-workflow)
...
- AI coding assistants (Copilot, ChatGPT, Claude) are genuinely useful

- But a 2025 MIT study finds a hidden cost: .hi[cognitive debt]

#### [MIT Media Lab, "Your Brain on ChatGPT" (2025)](https://www.media.mit.edu/publications/your-brain-on-chatgpt/)

- Participants using ChatGPT showed **significantly reduced frontal lobe engagement** (measured by EEG)
- Less recall of their own work; weaker sense of authorship: *"this doesn't feel like mine"*

--

#### What this means for coding

- You get working code, but you didn't build the mental model to understand it

- .hi[Cognitive debt accumulates:] each AI-assisted session adds code you don't fully understand

- Fine... until you need to modify, debug, or extend your code

- Eventually, your own codebase becomes a black box to you

---
# This lecture is your countermeasure

The workflow in this lecture **forces cognitive engagement**:

- .hi[Organization]: you designed the structure, you understand it
- .hi[Automation]: write the pipeline once yourself — know what each step does
- .hi[Abstraction rules]: only keep code you can explain to a classmate
- .hi[Documentation]: articulate what your code does in plain language
- .hi[Test-driven development]: define success *before* you write (or ask AI to write) code
- .hi[MREs]: isolate and understand problems yourself before handing them off

--

**Goal**: not to avoid AI, but to use it without surrendering understanding of your own work
...
# Agentic AI produces without a workflow

Without a structured workflow, agentic AI creates a new form of chaos:

<!-- Replace with a screenshot from one of your own AI-generated project folders -->
```bash
project/
├── analysis.R
├── analysis_v2.R
...
├── analysis_FINAL.R
├── analysis_FINAL_v2.R
├── analysis_FINAL_working_for_real.R   # which one is current?
├── helper_functions.R
├── helper_functions_refactored.R
├── utilities.R                          # same as above?
├── data.csv
├── data_clean.csv
├── data_clean_updated.csv
├── output.png
└── output_fixed.png
```

- AI has no concept of "clean up after yourself"
- Each new request generates a new file; old ones are never deleted
- No planned structure; no human understanding of what each piece does

.hi[Which `analysis_FINAL*.R` is the one that actually works?]
...
- When you are running a script, you are working from a particular directory
  ...
  - Your computer looks for a file called `file.csv` in the working directory
  - If that file is not in that directory, you will get a `FileNotFound` error
  - You can see what directory with `getwd()` in R, `pwd` in Stata/terminal function
    - Often listed above the coding console in RStudio, Stata, Jupyter, etc.
    - You can double click the `.Rproj` file to set the working directory to the root of the project
    - You can also change your working directory (`setwd()` in R, `cd` in terminal, `chdir` in Stata, etc.) but this is not recommended

```{r directories}
getwd()
```
```{r set-directory,eval=FALSE}
# new directory is the lectures/02-empirical-workflow folder
...
A "path" is the location of a file or directory on your computer. On my computer, the path to this file is:
...
This is called an .hi[absolute path]: long, hard to remember, not portable

Relative paths solve a lot of this:
  - The path to a file or directory starting from the current .hi[working directory]
  ...
  - **Portable**: on another computer with `big-data-class-materials`, it works!
...
- I have a folder called (`my_project`)
  - Within that folder I have subfolders:
      ...
      2. `code` for all code files, and sometimes:
          a. `code/build` for code files that build/clean code
          b. `code/analysis` for code files that do analysis
      3. `output` for all output files, _optionally_:
      ...
      5. `writing` for all writing files, _optionally_:
          a. `writing/notes` for notes
          b. `writing/drafts` for drafts
          c. `writing/edits` for edits
      6. `presentations` for all presentations, _optionally_:
          a. `presentations/slides` for slides
          b. `presentations/notes` for notes
- See [my starter project template](https://github.com/kgcsport/starter-project-repo) on GitHub as an example
...
- What share of NY population lives in county 36039? 

- What would a human say? What would a computer say? 

--

- **Human:** The share is 324920/43320903 = 0.0075
- **Computer:** The share is NA/43320903 = NA
...
- RDBM works in any modern statistical analysis software (R, Stata, SAS, Python, Julia, SQL, ...)

- Gentzkow & Shapiro suggest merging data sets as late as possible to avoid redundancy and mistakes


#### RDBM organizes data by unit of observation and avoids redundancy (and hence mistakes)
...
- This is somewhat aligned with the structure of an essay
    ...
    - Like essays revisions, modular code is easier to debug and revise
...
# File: main.R
# By: Kyle Coombs
# What: Runs the project from start to finish in R
# Date: 2024-09-0
# Instructions:
...
## Directory objects
data_dir <- file.path('data') # could instead be where your dropbox lives
...
for (y in 2004:2020) { # typically you'd use lapply in R, but this is more intuitive across languages
...
I copy/paste for each integer, but uh oh:

```{r round_to_nearest5}
rounded_157_nearest_6 <- round(157/5)*6
rounded_157_nearest_7 <- round(157/5)*7
```

```{r round_to_nearest5-display,echo=FALSE}
print(paste('rounded_157_nearest_6:', rounded_157_nearest_6, '; rounded_157_nearest_7:', rounded_157_nearest_7))
```
...
```{r round_to_nearest5_2}
round_to_nearest <- function(x,base=5) {
    return(round(x/base)*base)
}

rounded_157_nearest_6 <- round_to_nearest(157,base=6)
rounded_157_nearest_7 <- round_to_nearest(157,base=7)

```

```{r round_to_nearest5_2-display,echo=FALSE}
print(paste('rounded_157_nearest_6:', rounded_157_nearest_6, '; rounded_157_nearest_7:', rounded_157_nearest_7))
```
...
- If we're only doing it once in our script, then it may not make sense to write a function

- It can be difficult to know if one has reached the optimal level of abstraction

--

#### Note: AI abstracts like a (distracted) senior developer

- AI tools are trained on code written by experienced developers

- They naturally produce *senior-level* abstractions you may not yet recognize:
  - R6 classes, factory patterns, complex closures, generic utility frameworks
  - Full error handling, retry logic, logging, plugin interfaces

- The code does what you want. Until you need to change it. Then you have no idea where to look.

- This is abstraction working against you: it hides complexity you haven't mastered yet

#### Rule: only use abstractions you can explain to a classmate in 30 seconds
...
Basically, be empathetic and hold your own technical work to a higher standard. 
...
---
# Test-driven management with AI

When working with AI, .hi[define success before asking for code]

Write assertions first, then ask AI to write code that passes them:

```{r tdd_ai, eval=FALSE}
# STEP 1: Write assertions describing what success looks like
test_clean_wages <- function(df_raw, df_clean) {
  stopifnot(nrow(df_clean) == nrow(df_raw))     # no dropped rows
  stopifnot(all(!is.na(df_clean$wage)))          # no missing wages
  stopifnot(all(df_clean$wage > 0))              # wages are positive
  cat("All tests passed!\n")
}

# STEP 2: Tell AI: "write clean_wages() so test_clean_wages() passes"
# STEP 3: Run test_clean_wages(raw_data, clean_wages(raw_data))
# STEP 4: Ask AI: "walk me through clean_wages() line by line"
```

--

- Tests pass AND you can explain the code → proceed
- Tests pass but you can't explain the code → **slow down**
- Tests fail → you have a specific, concrete error and a ready-made MRE
...
---
# MREs make better AI prompts

Instead of pasting 500 lines and asking AI to "fix it", write an MRE first:

1. **Isolates the problem** — AI focuses on the issue, not 490 unrelated lines
2. **Becomes a test** — run it to verify AI's fix actually works
3. **You often find the bug yourself** while writing it (Postel's Law in action)

--

```{r bad_mre_for_ai, eval=FALSE}
# Bad AI prompt:
# "here is my 500-line script, something is wrong with the merge"
```

--

```{r good_mre_for_ai}
# Good AI prompt — write this MRE first:
library(dplyr)
df1 <- data.frame(id = c(1, 2, 3), wage = c(50, 60, 70))
df2 <- data.frame(id = c(1, 2, 4), industry = c("A", "B", "C"))
result <- left_join(df1, df2, by = "id")
# Expected: 3 rows, all with industry filled in
# Actual: id=3 has NA for industry — why?
nrow(result)
```

--

- Structured this way, AI's answer is **verifiable** and **targeted**
- Use `dput()` to share your data reproducibly: `dput(df1)`
