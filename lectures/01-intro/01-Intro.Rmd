...
- This course will also be **_rewarding_**
  ...
  - You will learn skills that employers, pre-doc programs, and grad schools want (but teach poorly)
...
- .hi[Data science (DS):] The scientific discipline that deals with transforming data into useful information ("insights") using a variety of stats/ML techniques
    ...
    - Facebook: Collects data on search history, friendship links, site clicks

    - Raj Chetty et al. used these data to estimate users' socioeconomic status and their social network ([Social Capital Atlas](https://www.socialcapital.org/))
...
--
  ...
  - A regression model with year fixed effects requires that rows vary by year
  - So you cannot have a wide data set with one row per person and one column per year

---
# What can we capture in data?

- Data can capture a lot of information about the world, but not everything
...
# How's the code run? 
.scroll-box-20[
```{r gapminder-chatgpt-eval,include=FALSE}
# Load necessary libraries
library(gapminder)
library(ggplot2)
...
- The aesthetics expect "long" data to differentiate series by a variable. This is distinct from the "wide" format, where each series has its own column.
...
- Here's are an additional examples to whet your appetite
...
In the meantime, I want youyou can do some reading and practice on your own. Pick either of the following (or choose among the litany of online resources) and work through their examples:
