...
3. Write code "modularly" (in pieces) to do each step, then you can troubleshoot modularly

4. The `help` documentation for R functions is the best place to start for troubleshooting

5. Google is your dictionary, AI is your weird friend who knows a lot of words but sometimes uses them wrong<sup>1</sup>
  - Be precise in your Google searches and ChatGPT instructions

  - I encourage you to use Generative AI ([ChatGPT](https://chat.openai.com/) and [GitHub CoPilot](https://github.com/features/copilot)) to help learn R and Git

6. If you're stuck, ask for help from me and classmates on GitHub Discussions/Issues
  - Write a minimally reproducible example of the bug
...
1. Exercises: low-stakes completition exercises

2. 5 group problem sets, drop the lowest, grade improvement allowed

3. Group presentations
  - 
4. GitHub Participation

5. Final Project/Lewiston Hack-a-thon: 2-day data science competition at end of semester
  - Prizes for "best" in various categories
  - Either week before Thanksgiving or last week of class -- come ready to vote next class
  - If falls through for some reason, final project will be a replication project
...
- .hi[Data science (DS):] The scientific discipline that deals with transforming data into useful information ("insights") using a variety of stats/ML techniques
    ...
    - Chetty et al. (2022) used FB data to estimate users' SES and social network ([Social Capital Atlas](https://www.socialcapital.org/))
...
- I'll encourage you to use Codespaces on some problem sets when I want to hit the ground running instead of troubleshooting package installation issues.
  - But you can use them for any problem set or exercise

- Use it today if your R installation is having issues!
...
abline(fit, col = "red") # Add in the object fit from before! 
```
...
Open up your laptops. For the remainder of this first lecture, we're going to play around with [ggplot2](https://ggplot2.tidyverse.org/) (i.e. livecoding) to explore our question about GDP and life expectancy. (Don't worry about working directories/file paths today, we'll cover that in the future.)

If you don't have them already, install the `ggplot2` and `gapminder` and `swirl` packages via either:
- **Console:** Enter `install.packages(c("ggplot2", "gapminder","swirl"), dependencies=T)`.
...
- Swirl is an R package that teaches you R interactively

- It's a great way to learn R if you're new to the language

- Use this code to get it up and running

```{r swirl_install,eval=FALSE}
library(swirl)
install_course_github('big-data-and-economics','ECON368-R-Swirls')
swirl()
```

- Tell swirl what it should call you (e.g. "Wizard of R", probably best to put your real name though)

- Then select the course `ECON368-R-Swirls` and the lesson `GGPlot Demo` (#13 last time I checked) and work through interactively

- At the end it will ask you to save your progress on Google. Do so! It will open Google in your browser, hit submit
  - It will also save a log file like 'swirl_log_author_Data Science for Economists_GGPlot2 Demo_.csv' in your working directory
  - You submitted an encrypted version of this file to me via Google (I'm demo-ing a GitHub version currently)
...
- It serves as a bridge between the abstract ideas in your head and code you write
...
---
# But do GDPPC increases cause life expectancy to increase?
