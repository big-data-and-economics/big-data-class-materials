...
date: "Bates College | [ECON368](https://github.com/ECON368-fall2023-big-data-and-economics/big-data-class-materials/tree/main)" #"`r format(Sys.time(), '%d %B %Y')`"
output:
...
We'll be using the following packages in this lecture:
...
```{r,echo=FALSE}
# This is here to reset the category variable which changed to NA at some point.
storms = mutate(storms, category=as.factor(ifelse(status=='hurricane', category,
                              ifelse(status=='tropical storm', 0, -1))))
```
...
The **tidyverse** is great. It will also provide a bridge to many of the big data tools that we'll encounter later in the course (SQL databases, etc.)
...
But first, a quick side-by-side comparison with dplyr, since that will help to orient us for the rest of the lecture. Using our starwars dataset, say we want to know:
...
# I need to assign then feed to ggplot manually, pipes were messing this up for some reason. Dependencies, amirite?
dt_key <- collapse_dt_key() 
ggplot(data=dt_key,aes(x = pressure, y = wind, col=category)) + 
  geom_point(alpha = 0.3) + 
  theme_minimal() +
  coord_equal()
```
