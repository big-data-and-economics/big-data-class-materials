...
- Please submit your project proposals if you have not already
...
☑ Did everyone fork the final project repo and submit a project proposal? (If not, please do so ASAP.)
...
We'll use **base** R today and then dive into the `tidyverse` and `data.table`
...
These slides are abridged

- The full slides for `R basics`, `tidyverse`, and `data.table` are available on the course website
...
- In Rmarkdown files, `<!--- --->` is the equivalent syntax for comments.
...
The good news, as we'll see, is that avoiding and solving these issues is pretty straightforward.
...
.footnote[<sup>1</sup> Guess who has two thumbs and has made this mistake on PS1? This guy.]
