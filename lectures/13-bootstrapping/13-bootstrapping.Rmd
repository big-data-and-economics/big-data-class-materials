...
```{r bootstrap-example-big-plot,echo=FALSE,warning=FALSE,cache=TRUE}
big_results <- big_bootstrap_results %>%
  ggplot(aes(x = forgiveness)) +
  geom_histogram(bins = 40) +
  geom_vline(xintercept = samp_big_effect, color = "red", size = 1) +
  geom_text(x=samp_big_effect-5, y=30, label="Sample estimate", hjust=0, angle=90, col="red") +
  geom_vline(xintercept = true_effect, color = "blue", size = 1) +
  geom_text(x=true_effect-5, y=30, label="True effect", hjust=0, angle=90, col="blue") +
...
- Take the 2.5th and 97.5th percentiles to get one type of 95% Confidence Interval<sup>3</sup>

```{r bootstrapping-density,echo=FALSE,warning=FALSE,fig.size='90%'}
...
big_results +
    ...
    geom_text(x=lwr-5, y=60, label="Lower 95% CI", hjust=0,angle=90, col="purple") +
    geom_vline(xintercept=upr, col="purple", linetype="dashed") +
    geom_text(x=upr-5, y=60, label="Upper 95% CI", hjust=0,angle=90, col="purple") +
