...
etable(savings,se.below=TRUE,fitstat=c('n')) %>% kable() # Make it prettier
```
...
samp1 <- pop %>% filter(had_loan == "No loan") %>% slice_sample(n = 500, by="forgiveness")
samp2 <- pop %>% filter(had_loan=="Had loan") %>% slice_sample(n = 500, by="forgiveness")
```

--

```{r savings-samples}
savings_new1 <- feols(savings ~ forgiveness, data = samp1) 
savings_new2 <- feols(savings ~ forgiveness, data = samp2) 
etable(savings,savings_new1,savings_new2, se.below=TRUE,fitstat=c('n')) %>% kable() # Make it prettier
```
...
coefficients <- bind_rows(savings$coefficients,savings_new1$coefficients,savings_new2$coefficients)
bind_rows(samp,samp1,samp2,.id="sample") %>% 
  group_by(sample) %>% 
  summarise(share_with_loan = mean(had_loan)) %>%
...
bootstrap_results %>%
  ...
  geom_text(x=samp_effect-30, y=60, label="Sample estimate", hjust=0, angle=90, col="red",size=12) +
  geom_vline(xintercept = true_effect, color = "blue", size = 1) +
  geom_text(x=true_effect-30, y=60, label="True effect", hjust=0, angle=90, col="blue",size=12) +
...
```{r bootstrap-example-big,echo=FALSE,warning=FALSE,cache=TRUE}
set.seed(1)
samp_big <- pop %>% slice_sample(n = 5000, replace = TRUE,by="forgiveness")
