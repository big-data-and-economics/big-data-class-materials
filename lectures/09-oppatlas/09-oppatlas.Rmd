...
- Can anyone think of examples of places where models are used to interpret data?

--

- Pre-1950s housing as a proxy for lead exposure
...
```{r autocovariance, echo=F, out.width="80%"}
...
```{r mobility_trends, echo=F, out.width="90%",warning=FALSE,message=FALSE}
...
diff <- tidy(results) %>% filter(term=='kfr_p25_demean') %>% pull(estimate) - corr

etable(results,note=paste('Correlation function and z-scored regression approach are within', 
  round(diff,16), 'of each other'))
```
